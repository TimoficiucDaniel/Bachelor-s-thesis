...
public static class SimplexNoise
{
    private static readonly double[][] grad3 = {
        new double[] {1, 1, 0}, new double[] {-1, 1, 0}, new double[] {1, -1, 0},
        new double[] {-1, -1, 0}, new double[] {1, 0, 1}, new double[] {-1, 0, 1},
        new double[] {1, 0, -1}, new double[] {-1, 0, -1}, new double[] {0, 1, 1},
        new double[] {0, -1, 1}, new double[] {0, 1, -1}, new double[] {0, -1, -1}
    };
    ...
    public static float noise(double xin, double yin)
    {
        ...
        double y0 = yin-Y0; 
        // Get third point, since we are in 2d 
        int offsetI, offsetJ; //offsets for third point
        if (x0 > y0)
        {// lower triangle
            offsetI = 1;
            offsetJ = 0;
        ...
        double x1 = x0 - offsetI + G2; // Offsets for middle corner in (x,y) unskewed coords
        double y1 = y0 - offsetJ + G2;
        double x2 = x0 - 1.0 + 2.0 * G2; // Offsets for last corner in (x,y) unskewed coords
        double y2 = y0 - 1.0 + 2.0 * G2;
        int ii = i & 255;
        int jj = j & 255;
        int gi0 = permMod12[ii+perm[jj]];
        int gi1 = permMod12[ii+offsetI+perm[jj+offsetJ]];
        int gi2 = permMod12[ii+1+perm[jj+1]];
        double t0 = 0.5 - x0*x0-y0*y0;
        if(t0<0) corner0 = 0.0;
        else {
            t0 *= t0;
            corner0 = t0 * t0 * dot(grad3[gi0], x0, y0);  // (x,y) of grad3 used for 2D gradient
        }
        double t1 = 0.5 - x1*x1-y1*y1;
        if(t1<0) corner1 = 0.0;
        else {
            t1 *= t1;
            corner1 = t1 * t1 * dot(grad3[gi1], x1, y1);
        }
        double t2 = 0.5 - x2*x2-y2*y2;
        if(t2<0) corner2 = 0.0;
        else {
            t2 *= t2;
            corner2 = t2 * t2 * dot(grad3[gi2], x2, y2);
        }
        // Add contributions from each corner to get the final noise value.
        // The result is scaled to return values in the interval [-1,1].
        return (float)(70.0 * (corner0 + corner1 + corner2));
    }
}